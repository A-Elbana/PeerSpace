...
const isUserInCommunity = async (userId: number, communityId: number): Promise<boolean> => {
...
const isInstructorOfCommunity = async (userId: number, communityId: number): Promise<boolean> => {
    const manages = await prisma.manages.findUnique({
        where: {
            iid_cid: {
                iid: userId,
...
export const createPost = async (req: Request, res: Response) => {
    ...
    // CID can come as string or number depending on parser but req.body usually has correct types if JSON
    // But we safeguard
...
export const getPostById = async (req: Request, res: Response) => {
    ...
    try {
        ...
        // Validate membership in the community of the post
        const isMember = await isUserInCommunity(userId, post.cid);

        const role = (req as any).role;
        if (!isMember && role !== "ADMIN") {
            return res.status(403).json({ message: "You are not a member of the community this post belongs to" });
        }
...
export const getPostsByCommunity = async (req: Request, res: Response) => {
    ...
    // Validate membership
    const isMember = await isUserInCommunity(userId, cid);
    const role = (req as any).role;
...
export const updatePost = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (post.owner_uid !== userId && role !== "ADMIN") {
            return res.status(403).json({ message: "Only the author or an admin can update this post" });
        }
...
export const togglePostResolved = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (post.owner_uid !== userId && !isInstructor && role !== "ADMIN") {
            return res.status(403).json({ message: "You are not authorized to resolve this post" });
        }
