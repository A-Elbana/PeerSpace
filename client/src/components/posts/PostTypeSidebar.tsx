...
const PostTypeSidebar = ({ config, children }: PostTypeSidebarProps) => {
  ...
  return (
    ...
    >
      {children || (
        <div className={`w-8 h-8 rounded-full ${config.iconBgColor} flex items-center justify-center shadow-lg ${config.iconColor.includes('yellow') ? 'shadow-yellow-500/30' : config.iconColor.includes('blue') ? 'shadow-blue-500/30' : 'shadow-turf-green-500/30'}`}>
