...
const PostPreview: React.FC = () => {
  ...
  // Recursive renderer
  const CommentItem: React.FC<{ comment: Comment; depth?: number; renderChildren?: boolean }> = ({ comment, depth = 0, renderChildren = true }) => {
            ...
            {comment.replies && comment.replies.length > 0 && renderChildren && (
              <div className="mt-4 space-y-3">
                {comment.replies.map(r => (
                  <div key={r.id}>
                    <CommentItem comment={r} depth={(depth || 0) + 1} renderChildren={expandedCommentIds.has(r.id)} />
